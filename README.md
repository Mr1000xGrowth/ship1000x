...
[![Status: Beta v1.1.0](https://img.shields.io/badge/status-beta%20v1.1.0-c8a464.svg)]()

![Ship1000x dashboard demo](docs/assets/ship1000x-demo.gif)
...
- **Output classified** : lines committed split into `real` / `seed` /
  ...
  diff.
- **API-equivalent LLM cost** : token-based estimates from rollouts (Anthropic +
  OpenAI pricing baked in), separated from billed-estimated and
  subscription-absorbed cost.
...
In beta (`v1.1.0`), install from source :
...
| Claude Code statusline | `~/.ship1000x/drop/statusline/` (live, opt-in) | Per-minute context-window usage, dominant model, compact boundaries |
| Codex CLI | `~/.codex/sessions/**/rollout-*.jsonl` | Sessions, token usage, model used |
| Codex Desktop | `~/.codex/state_5.sqlite` logs | Sessions/turns metadata, tokens unknown |
| Codex macOS app | `~/Library/Logs/com.openai.codex/` | Turn starts, approvals, tokens unknown |
| Cursor | `~/.cursor/ai-tracking/ai-code-tracking.db` | AI block metadata, tokens/cost unknown |
| Cline | VS Code/Cursor extension task metadata | Task metadata, tokens/cost unknown |
...
| Agent runtime (live) | `ship1000x watch` foreground daemon (opt-in, `[watch]` extra) | AI processes + listening TCP ports + MCP servers, polled every 30s |
| Local usage proxy | `~/.ship1000x/drop/trace/` (opt-in) | Anthropic + OpenAI API usage metadata captured by an optional separate local proxy that drops JSONL records here |

All collectors are **read-only**. Nothing is ever modified, deleted, or
transmitted from your machine without explicit opt-in.

The last two rows are the Wave 3 / Wave 4 additions: they capture live
runtime signals (process inventory, listening ports, MCP servers) and
provider-side API metadata (tokens, model used, endpoint) that the
batch JSONL collectors above cannot see. Both are opt-in via separate
install steps and never touch the network unless you ask them to.

Use `ship1000x source-audit` to see which observed sources are audit-grade,
partial, fragile, unknown, or missing normalized usage metadata. The audit also
shows each source readiness stage (`default_ingest`, `opt_in_ingest`,
`billing_api`, `drop_import`, `fixture_only`, and so on), so fixture-only or
registry-only rows cannot be mistaken for real local observation. Use
`ship1000x observation-audit` to see the broader capability map: what SHIP
observes today, what remains partial, and what belongs to future
live-capture tools. See [docs/SOURCE_QUALITY.md](docs/SOURCE_QUALITY.md) and
[docs/OBSERVATION_AUDIT.md](docs/OBSERVATION_AUDIT.md).

`observation-audit` also names known provider/tool gaps from the open-source
audit, such as Gemini CLI, Copilot, OpenCode, Roo/Kilo Code variants, Cursor
Agent, Continue, Aider, Antigravity, Goose, Kimi, and Qwen. These are visible
claim boundaries, not hidden TODOs.

The first provider-expansion entries are now registered conservatively in
`source-audit`: Gemini CLI, Copilot agents, OpenCode, Cursor Agent, Continue,
and Aider are fixture-only until read-only collectors are backed by real local
format validation. Roo/Kilo Code is wired into normal ingest with its own
source identity and remains conservative on token/cost quality. Public gates
fail when fixture-only or registry-only stages are observed.

`source-audit --json` also exposes per-source quality scores for token, cost,
active-time and overall measurement truth. Treat those scores as claim hygiene:
they explain whether a number is factual, defensible, indicative, unknown, or
not measured before it appears in a public dashboard or report.
It also exposes safe auth-mode counts and a cost-truth split so API-equivalent
cost, billed-estimated cost, and subscription-absorbed cost are not confused.

The local dashboard overview surfaces the same scorecard under `Measurement
quality`, so public demos can show not only what was measured, but how much
confidence each source deserves.

Before using SHIP as a public proof point, run `ship1000x source-audit
--strict-public`. It exits non-zero when observed source quality is too low,
when fragile/unknown sources would make the claim unsafe, or when an observed
row came from a fixture-only, registry-only, or collector-module-only stage.

For a single pre-demo gate, run `ship1000x public-check`. It combines the
strict source-quality gate with the observation capability audit and reports
one of three presentation states: `PASS`, `CAVEAT`, or `NO GO`. `PASS` means
no proof caveats were found, `CAVEAT` means the demo/report is usable only
with visible caveats, and `NO GO` means blocking source-quality or history
failures must be fixed first.
Its JSON output includes `public_claim_readiness`, which separates claims that
are safe to quote from claims that must stay blocked or caveated, such as
complete provider coverage, invoice-grade cost truth, and live runtime truth.
It also includes `public_proof_sequence`, the rerunnable command sequence with
per-step status, aggregate evidence, caveats, and quote policy.
Machine consumers should read `proof_status` / `presentation_status`; the
legacy `passed` boolean is kept only as an exit-code/gate compatibility field
and can be true while `proof_status` is `caveat`.

If the public claim depends on older rows being repairable or defensible, run
`ship1000x public-check --strict-history` instead. It turns partial,
external-source-required, not-recoverable, or malformed history into blocking
failures. The recommended public-proof sequence is:
...
🚀 Highlights — derniers 30 jours
  ...
  Coût API-equivalent          $5 396      [93% native token/pricing coverage; remainder indicative/unknown]
  Trust Score                  96/100      Factual · weighted avg per source
  Robustness checks            3/3         reported separately, never added
```
...
source) + Projects (sortable matrix project × tool × API-equivalent cost). Window selector
...
- **3-ratios overview redesign** — the dashboard now reports R1 (Time leverage,
  ...
  team, Indicative · user input) as separate sections with explicit confidence
...
- **`ship1000x summary`** — cross-tab matrix : per-project breakdown by
  tool (Claude Code, Codex, Cursor, …) + dominant tool + API-equivalent cost.
  Filter by `--client <name>` if you tag projects.yaml with `client:`.
- **Trust Score** — every metric carries its confidence level. Per-source
  scoring (high=100, medium=70, low=40) + raw weighted global average.
  Cadence, daily_unified, and critical-source checks are reported separately as
  robustness checks, never added to the score.
...
ship1000x source-audit             # source truth audit: coverage, usage metadata, risk
ship1000x observation-audit        # capability + provider gap audit
ship1000x history-audit            # read-only historical recoverability matrix
ship1000x reclassify --dry-run     # preview historical repair impact before mutation
ship1000x public-check             # one-command public demo/report gate
ship1000x public-check --strict-history  # block public claims on unrecoverable history
...
│                                                                              │
│   Production réelle            607 008   lignes vrai code (92%) [Factual]    │
│   Coût API-equivalent          $5 396      [93% native token/pricing]        │
│   API-eq / ligne nette         $0.0089   total API-eq / net line             │
│                                                                              │
│   Trust Score                  96/100     Factual · weighted avg per source  │
...
each labelled with the current confidence taxonomy: Factual / Defensible / Indicative / Unknown. Window selector
7 / 14 / 30 / 60 / 90 / 180 / 365 days.
The local JSON API contract is documented in
[`docs/DASHBOARD_API.md`](docs/DASHBOARD_API.md), including cost-truth fields
that separate API-equivalent, billed-estimated, subscription-absorbed, and
unknown-basis cost.
...
`my-project`). Estimates persist per-project in localStorage.
...
- [x] Trust Score per source + raw weighted global score (the differentiator)
