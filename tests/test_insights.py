...
def _usage_meta(api_equivalent_cost: float) -> str:
    return json.dumps({
        "usage": {
            "auth_mode": "oauth",
            "cost": {
...
class TestEngineOverview(unittest.TestCase):
    ...
    def test_overview_cost_uses_cost_truth_api_equivalent(self):
        s = _make_storage()
        ts = datetime.now(timezone.utc) - timedelta(hours=2)
        _insert_event(
            s,
            ...
            duration_sec=3600,
            cost_estimated=99.0,
            raw_meta=_usage_meta(api_equivalent_cost=4.0),
        )
        _insert_event(
            s,
            source="git",
            ...
            raw_meta=json.dumps({
                ...
                "lines_deleted": 0,
                "files_changed": 1,
                "lines_real_added": 100,
                "lines_real_deleted": 0,
            }),
            project_id="test",
        )
        w = Window(
        ...
        self.assertEqual(ov["totals"]["cost"], 4.0)
        self.assertEqual(ov["ratios"]["cost_per_commit"], 4.0)
        self.assertEqual(ov["ratios"]["cost_per_hour"], 4.0)
        self.assertEqual(ov["ratios"]["cost_per_line_net"], 0.04)
...
class TestMultiplier(unittest.TestCase):
    ...
    def test_multiplier_cost_uses_cost_truth_api_equivalent(self):
        ...
        self.assertEqual(m["cost"]["total_usd"], 4.0)
        self.assertEqual(m["cost"]["per_hour_usd"], 4.0)
        self.assertEqual(m["cost"]["per_commit_usd"], 4.0)
        self.assertEqual(m["cost"]["per_line_net_usd"], 0.04)
...
class TestSignalsCostTruth(unittest.TestCase):

    def test_cost_spike_uses_cost_truth_api_equivalent(self):
        s = _make_storage()
        now = datetime.now(timezone.utc).replace(hour=12, minute=0, second=0, microsecond=0)
        samples = [
            (now - timedelta(days=3), 100.0, 1.0),
            (now - timedelta(days=2), 100.0, 1.0),
            (now - timedelta(days=1), 1.0, 10.0),
        ]
        for index, (ts, stored_cost, api_cost) in enumerate(samples):
            _insert_event(
                s,
                id=f"cost-{index}",
                started_at=ts.isoformat(),
                cost_estimated=stored_cost,
                raw_meta=_usage_meta(api_equivalent_cost=api_cost),
            )
        w = Window(since=now - timedelta(days=5), until=now)

        signals = detect_cost_spike(s, w)

        self.assertEqual(len(signals), 1)
        self.assertEqual(signals[0]["type"], "cost_spike")
        self.assertEqual(signals[0]["data"]["avg_daily_cost"], 4.0)
        self.assertEqual(signals[0]["data"]["spikes"], [
            {"day": (now - timedelta(days=1)).strftime("%Y-%m-%d"), "cost": 10.0}
        ])
