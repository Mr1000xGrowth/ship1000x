...
class TestGetAllSourceScores(unittest.TestCase):
    ...
    def _insert(self, source: str, confidence: str, n: int = 1, raw_meta: dict | None = None):
        ts = datetime.now(timezone.utc).isoformat()
        with self.storage.conn() as c:
            for i in range(n):
                c.execute(
                    """INSERT INTO events
                       (id, source, event_type, started_at, confidence_flag, raw_meta, machine_id)
                       VALUES (?, ?, 'session', ?, ?, ?, 'test')""",
                    (
                        f"{source}-{confidence}-{i}",
                        source,
                        ts,
                        confidence,
...
class TestComputeGlobalScore(unittest.TestCase):
    ...
    def test_source_quality_check_fails_when_high_confidence_lacks_usage_metadata(self):
        self._insert("claude_code", "high", n=5)
        result = compute_global_score(self.storage)

        # The score remains raw confidence, but robustness exposes the audit gap.
        self.assertEqual(result["score"], 100)
        chk = self._check(result, "Source quality audit")
        self.assertFalse(chk["passed"])
        self.assertIn("fragile=1", chk["detail"])
        self.assertIn("missing_usage=1", chk["detail"])

    def test_source_quality_check_passes_for_factual_usage_metadata(self):
        self._insert(
            "codex",
            "high",
            raw_meta={
