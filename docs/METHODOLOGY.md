...
3. [Cost — API-equivalent pricing + cache tokens](#3-cost--api-equivalent-pricing--cache-tokens)
...
## 3. Cost — API-equivalent pricing + cache tokens

### The calculation

For sources that expose tokens (claude_code, codex, openclaw, anthropic_usage, openai_usage):
...
Pricing is maintained in `core/pricing.py` as a local snapshot of published
Anthropic / OpenAI / Google rate cards. It is the source for SHIP's
API-equivalent estimates, not proof of what a subscription, quota, discount,
tax, credit, or custom contract actually billed.
...
Patches `fix(claude_code) 2343626` correct these 2 bugs. See [migration cookbook](../README.md) to propagate the fix to historical data safely: run `ship1000x history-audit --since 365d`, inspect `ship1000x reclassify --since 365d --dry-run`, then run `ship1000x reclassify --since 365d` only if the original local source files still exist.
...
ship1000x reconcile --since 30d
```

It compares SHIP's local API-equivalent totals with billing-side rows imported
from provider Admin/Usage APIs. Treat the verdict as a reconciliation signal,
not as a standalone invoice certificate: provider exports can lag, exclude
OAuth/subscription seats, or use account-specific billing semantics.
...
Documented as **Indicative** in `ship1000x insights`. For numerical validation,
cross-reference with `openai_usage` provider-export rows when explicitly
available.
...
| `cost_per_line_net` | `api_equivalent_cost / max(1, real_net)` | `cost_per_line_net_raw` |
...
multiplier = {
    ...
    "cost_per_hour": api_equivalent_cost / active_hours, # API-equivalent pace
...
In `ship1000x highlights`, when computing the leverage multiplier
...
- `ship1000x highlights` displays the capped multiplier (defensible WOW number)
- `ship1000x insights` displays both the raw and the unified-source multiplier
...
| No token estimation for Cline / Cursor / Codex Desktop | Current metadata is not invoice-grade. Store `unknown`, not fake zeros or confident estimates. |

### What we DISPLAY transparently

In every `ship1000x today` or `ship1000x insights` report:
...
4. The **confidence level** per source and metric (Factual / Defensible / Indicative / Unknown)
5. The **observation capability status** when using `ship1000x observation-audit`
...
- Allow provider-export validation (`anthropic_usage` Admin API + reconcile.py)
- Display confidence levels per metric
- Separate source quality from broader observation coverage
