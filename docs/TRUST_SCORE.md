...
| Source quality audit | No observed source is fragile, partial, unknown, or missing normalized usage metadata | `confidence_flag` is backed by explicit token/cost/active-time quality |
...
| < 60 | Calibration needed | Run `ship1000x calibrate`, check missing sources, run reconciliation |

---

## How it appears in reports

### `ship1000x insights` (V1.1)
...
API-equivalent cost          $1,247          ██████████  Native token/pricing (97)
Billed reconciliation        ±3.2%           ██████████  Provider export required
...
### `ship1000x calibrate` output

Already implemented. Shows the user's cadence profile + classification. The
calibration does **not** inflate the global score; it is reported separately as
a robustness check.
...
| Symptom | Cause | Fix |
|---|---|---|
| `claude_code` score < 90 | Cache tokens missing in old events (pre-V1 patch) | Run `ship1000x history-audit --since 365d`, then `ship1000x reclassify --since 365d --dry-run`, then reclassify only if source files still exist |
| `git` score < 80 | Many files unclassified by line_classifier | Review `~/.config/ship1000x/line_classifier.yaml` patterns |
| `codex_macapp` score = 70 | Cost via heuristic and tokens explicitly `unknown` | Cross-check with `openai_usage` for validation |
...
| Robustness check "Source quality audit" failing | Some observed sources are fragile/partial/unknown or missing `raw_meta.usage` | `ship1000x source-audit --since 30d`, then fix the listed collector or quote with caveats |
...
4. **A way to cross-check it** (reconcile with provider billing-side exports) + 
...
| V1.1 | `ship1000x insights` displays score per metric + global composite score |
| V1.2 | Per-metric explanation (`ship1000x explain lines_per_hour`) shows the heuristic used + sample data |
