...
Last updated: 2026-05-27 (V1.1 source/privacy hardening).
...
The local SQLite DB follows the XDG data directory. By default it lives at
`~/.local/share/ship1000x/tracker.sqlite` on Linux/macOS unless
`XDG_DATA_HOME` is set. It contains:
...
If you want to share aggregated metrics with collaborators via a Garage S3
bucket you control, Ship1000x provides `ship1000x push-insights` with a
**conservative filter** by default.
...
## Collectors that touch external APIs

Most collectors read **only local files**. The billing collectors below connect
to external APIs only with explicit credentials/approval:
...
Manual web exports are different: `web_exports` reads local files you place in
the drop folder and never calls a provider API. The current parser keeps
aggregate conversation metadata only; token/cost truth remains unknown unless a
provider usage source is separately reconciled.
...
```bash
ship1000x push-insights --dry-run --since 7d
```
