...
def detect_cost_spike(storage, window: Window) -> list[dict[str, Any]]:
    ...
    signals: list[dict[str, Any]] = []
    rows = storage.query(
        """SELECT DATE(started_at) AS d, cost_estimated, raw_meta
             FROM events
             WHERE started_at >= ? AND started_at < ?
             ORDER BY d""",
        (window.since_iso, window.until_iso),
    ...
    for r in rows:
        ...
        if not day:
            continue
        by_day[day] = by_day.get(day, 0.0) + api_equivalent_cost_from_row(r)

    costs_by_day = [(d, c) for d, c in sorted(by_day.items()) if c > 0]
    if len(costs_by_day) < 3:
        return signals
    costs = [c for _, c in costs_by_day]
    avg = sum(costs) / len(costs)
    spikes = [(d, c) for d, c in costs_by_day if c > 2 * avg]
    if spikes:
        signals.append({
            "category": "cost",
            "type": "cost_spike",
