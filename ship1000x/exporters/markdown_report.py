...
from ..core.cost_truth import api_equivalent_cost_from_row
...
def _api_equivalent_cost(row) -> float:
...
def generate_report(storage, cutoff: datetime, since_label: str = "") -> str:
    ...
    total_cost = sum(_api_equivalent_cost(r) for r in event_cost_rows)
    cost_measured = sum(_api_equivalent_cost(r) for r in event_cost_rows if not is_hourly_estimated(r["source"]))
    cost_hourly = sum(_api_equivalent_cost(r) for r in event_cost_rows if is_hourly_estimated(r["source"]))
    ...
    if cost_hourly > 0 and cost_measured > 0:
        lines.append(
            f"- **Cout API-equivalent estime** : ${total_cost:.2f} "
            f"(${cost_measured:.2f} mesure tokens reels · "
            f"${cost_hourly:.2f} estime horaire apps fermees)"
        )
    else:
        lines.append(f"- **Cout API-equivalent estime** : ${total_cost:.2f}")
    ...
    lines.append("")
    lines.append("| Projet | Temps actif | Events | Tokens | Cout API-eq $ |")
    lines.append("|---|---:|---:|---:|---:|")
    ...
    try:
        ...
        lines.append(
            f"- **Cout API-equivalent IA** : ${c['total_usd']} total · "
