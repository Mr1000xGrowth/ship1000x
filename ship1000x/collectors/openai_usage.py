...
def collect(
    ...
    for bucket in usage_buckets:
        ...
        for r in results:
            ...
            uncached_in = max(int(tokens_in) - int(cached_in), 0)
            total_tokens = tokens_in + tokens_out

            # Cost is computed from tokens * pricing.py for consistency
            # with the Anthropic side (same fix). The cost endpoint that
            # the OpenAI Admin API exposes returns a single aggregate per
            # bucket whose unit / inclusion rules are not documented; the
            # token*pricing path is fully auditable from the public rate
            # card and matches what every other SHIP collector reports.
            ...
            if day_total_tokens > 0 and day_cost > 0:
                api_cost_usd = day_cost * (total_tokens / day_total_tokens)
                ...
                "raw_meta": json.dumps(
                    {
                        ...
                        # api_reported_cost = what the OpenAI Admin API
                        # cost endpoint returned (after token-prorata).
                        # Kept for audit; not used for cost_estimated
                        # because token*pricing is the auditable source
                        # of truth here, like Anthropic.
                        "api_reported_cost": round(api_cost_usd, 8),
                        # V7 model_stats: per-model breakdown including cache
                        # and the uncached input so reports do not need to
                        # peek at top-level counters.
                        "model_stats": {
                            model: {
                        ...
                        # OpenAI billing aggregates OAuth + API key usage on
                        # the provider side. For SHIP's cost-honesty contract
                        # we tag billing snapshots as `api_key` (what the
                        # user pays today) and keep `billing_source` for
                        # detail.
                        "billing_source": "billing_aggregated",
                        "auth_mode": "api_key",
                        "usage": build_usage_metadata(
                            provider="openai",
                            client="openai-usage-api",
                            model_raw=model,
                            ...
                            cost_basis="tokens_times_published_openai_pricing",
                            token_source="openai_admin_usage_completions",
                            auth_mode="api_key",
                        ),
                    }
                ),
