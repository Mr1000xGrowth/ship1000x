...
def collect(storage, classifier, privacy_config: dict[str, Any]) -> dict[str, int]:
    from ship1000x.core.gitleaks_scan import (
        is_enabled as gitleaks_enabled,
    )
    from ship1000x.core.gitleaks_scan import (
        is_gitleaks_available,
    )
    from ship1000x.core.gitleaks_scan import (
        scan_repo as gitleaks_scan_repo,
    )
    ...
    # Gitleaks integration is strictly opt-in via privacy.yaml (or the
    # `share_config` dict). Disabled by default — secret-scanning a
    # large set of repos can take minutes and the user must consent
    # to having alert events stored in the local DB.
    run_gitleaks = gitleaks_enabled(privacy_config) and is_gitleaks_available()
    gitleaks_config_path = None
    if run_gitleaks:
        # Optional user-local baseline that mutes known false positives.
        # Convention: ~/.config/ship1000x/.gitleaks.toml — same dir as
        # privacy.yaml. If absent, gitleaks runs with its built-in rules.
        candidate = (
            Path.home() / ".config" / "ship1000x" / ".gitleaks.toml"
        )
        if candidate.exists():
            gitleaks_config_path = candidate
    ...
    for repo in _find_git_repos(DEFAULT_SCAN_ROOTS):
        ...
        # Optional gitleaks pass — emits one event per finding, with
        # a unique stable id so repeated ingestion does not duplicate
        # the alert. Sanitised by `gitleaks_scan._sanitize_finding`:
        # no matched secret text, no committer name/email, file
        # basename only.
        if run_gitleaks:
            findings = gitleaks_scan_repo(
                repo, config_path=gitleaks_config_path
            )
            for finding in findings:
                alert_id = _stable_event_id(
                    repo_key,
                    f"gitleaks|{finding['commit']}|{finding['rule_id']}|{finding['file']}|{finding['line']}",
                )
                alert_event = {
                    "id": alert_id,
                    "source": "git_secret_alert",
                    "event_type": "secret_alert",
                    "started_at": finding["date"] or datetime.utcnow().isoformat(),
                    "ended_at": finding["date"] or datetime.utcnow().isoformat(),
                    "duration_sec": 0,
                    "cwd": repo_str,
                    "project_id": project_id,
                    "project_conf": conf,
                    "tool_or_action": "gitleaks_finding",
                    "token_input": 0,
                    "token_output": 0,
                    "cost_estimated": 0.0,
                    "user_msg_type": None,
                    ...
                    "raw_meta": json.dumps({
                        ...
                        "gitleaks_rule_id": finding["rule_id"],
                        "gitleaks_description": finding["description"],
                        "gitleaks_file": finding["file"],
                        "gitleaks_line": finding["line"],
                    }),
                }
                safe = sanitize_event(alert_event)
                storage.upsert_event(safe)
                stats["gitleaks_alerts"] += 1
