...
def _provider_for_model(model: str | None) -> str:
    """Best-effort provider label from Cline's model id, without over-claiming."""
    ...
    if ui_file.exists():
        try:
            ...
            for msg in ui:
                ...
                if (
                    (msg.get("type") in ("say", "ask") and msg.get("say") == "user_feedback")
                    or msg.get("type") == "ask"
                ):
                    if ts := msg.get("ts"):
...
def collect(storage, classifier, privacy_config: dict[str, Any]) -> dict[str, int]:
    ...
    for task_dir in CLINE_TASKS_DIR.iterdir():
        ...
        storage.set_ingestion_offset(
            "cline", task_dir.name,
            current_mtime, datetime.now(timezone.utc).isoformat(),
        )
