"""Fixture-only OpenCode session parser.

The parser locks a safe metadata contract for future provider expansion. It is
not wired to `ship1000x ingest` and never discovers real OpenCode files.
"""
...
SOURCE = "opencode"
SCHEMA_VERSION = "ship1000x.opencode.session.v1"
...
def parse_session_file(path: Path) -> dict[str, Any]:
    """Parse a redacted OpenCode fixture into safe event metadata."""
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError(f"Unsupported OpenCode fixture schema: {payload.get('schema_version')}")
    ...
    if tokens.has_native_tokens:
        usage = build_usage_metadata(
            ...
            token_source="opencode_fixture_usage",
        )
    else:
