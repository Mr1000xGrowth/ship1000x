...
def _primary_model(model_stats: dict[str, dict[str, Any]]) -> str:
    """Return the dominant model by turns, or 'mixed' when ties differ."""
    if not model_stats:
        return "unknown"
    ordered = sorted(
        model_stats.items(),
        key=lambda item: (item[1].get("turns", 0), item[1].get("tokens_in", 0)),
        reverse=True,
    )
    top_model, top_stats = ordered[0]
    if len(ordered) > 1 and ordered[1][1].get("turns", 0) == top_stats.get("turns", 0):
        return "mixed"
    return top_model


def _build_daily_usage_metadata(day_stats: dict[str, Any], ratio: float) -> dict:
    """Build safe Claude Code usage metadata for one stored session_day event."""
    input_uncached = int(day_stats.get("tokens_input_uncached", 0) * ratio)
    output_tokens = int(day_stats.get("tokens_output", 0) * ratio)
    ...
    auth_mode = detect_claude_auth_mode(day_stats.get("entrypoint"))
    return build_usage_metadata(
        provider="anthropic",
        client="claude-code",
        model_raw=_primary_model(day_stats.get("model_stats") or {}),
        ...
        token_source="claude_message_usage",
        auth_mode=auth_mode,
    )
...
def parse_session_file(path: Path) -> dict[str, Any]:
    ...
    daily: dict[str, dict[str, Any]] = defaultdict(lambda: {
        ...
        # Compact-summary events. Claude Code emits a record with
        # ``isCompactSummary: true`` every time the session is
        # `/compact`-ed (manually or automatically by the runtime when
        # the context window fills up). Tracking these lets a
        # downstream report tell pre-compact vs post-compact work
        # apart and surfaces when a long session was forced to
        # summarise its own context.
        "compact_count": 0,
        "compact_timestamps": [],
        ...
        # per day and kept stable (a session never switches client mid-run).
        "entrypoint": None,
    })
    ...
    try:
        with path.open("r", encoding="utf-8") as f:
            for line_num, line in enumerate(f):
                ...
                if ts:
                    d["last_ts"] = ts
                # Capture entrypoint once per day. Claude Code writes the
                # same value on every record of a session.
                if not d.get("entrypoint"):
                    ep = record.get("entrypoint")
                    if isinstance(ep, str) and ep.strip():
                        d["entrypoint"] = ep.strip()

                # Compact summaries are user-typed records with a
                # `isCompactSummary: true` marker. We count them
                # before the regular user-event classification so the
                # synthetic /compact record does not skew the
                # typed/paste/tool_result distribution.
                if rec_type == "user" and record.get("isCompactSummary") is True:
                    d["compact_count"] += 1
                    if ts:
                        d["compact_timestamps"].append(ts)
                    continue
                ...
                if rec_type == "user":
                    ...
                    ms["cache_write_tokens"] += cache_create
                    ms["cost"] += cost_here
                    ms["turns"] += 1
            ...
            for pid, ratio in distribution.items():
                ...
                event = {
                    ...
                    "raw_meta": json.dumps({
                        ...
                        # Compact-summary tracking. `compact_count` is the
                        # number of `/compact` events that fired on this
                        # day; `compact_first` / `compact_last` are the
                        # ISO timestamps of the first and last compact
                        # for the day. Both are derived from
                        # `isCompactSummary: true` records in the JSONL.
                        "compact_count": int(d.get("compact_count", 0)),
                        "compact_first": (
                            d["compact_timestamps"][0]
                            if d.get("compact_timestamps")
                            else None
                        ),
                        "compact_last": (
                            d["compact_timestamps"][-1]
                            if d.get("compact_timestamps")
                            else None
                        ),
                        ...
                        "model_stats": {
                            m: {
                                ...
                                "cache_read_tokens": int(s.get("cache_read_tokens", 0) * ratio),
                                "cache_write_tokens": int(s.get("cache_write_tokens", 0) * ratio),
                                "cost": s["cost"] * ratio,
