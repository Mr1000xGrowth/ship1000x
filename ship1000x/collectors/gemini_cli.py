"""Fixture-only Gemini CLI session parser.

This module deliberately does not discover or read real Gemini CLI logs. It
defines the minimal redacted session contract SHIP needs before a future
read-only collector can be implemented safely.
"""
...
SOURCE = "gemini_cli"
SCHEMA_VERSION = "ship1000x.gemini_cli.session.v1"
...
def parse_session_file(path: Path) -> dict[str, Any]:
    """Parse a redacted Gemini CLI fixture without preserving raw content."""
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError(f"Unsupported Gemini CLI fixture schema: {payload.get('schema_version')}")
