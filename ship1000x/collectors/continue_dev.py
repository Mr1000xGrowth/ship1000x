"""Fixture-only Continue (continue.dev) session parser.

Continue is a multi-provider code assistant. The same client can route to
OpenAI, Anthropic, Gemini, or local models depending on configuration, so
token/cost truth depends on the underlying provider response. This parser
captures only safe session metadata from a redacted synthetic fixture and
never discovers real Continue session files. It is not wired to
`ship1000x ingest`.

The module is named `continue_dev` because `continue` is a Python reserved
word; the SHIP source identifier remains the string `"continue"`.
"""
...
SOURCE = "continue"
SCHEMA_VERSION = "ship1000x.continue.session.v1"
...
def parse_session_file(path: Path) -> dict[str, Any]:
    """Parse a redacted Continue session fixture into safe event metadata."""
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise ValueError(
            f"Unsupported Continue fixture schema: {payload.get('schema_version')}"
        )
    ...
    if tokens.has_native_tokens:
        usage = build_usage_metadata(
            ...
            token_source="continue_fixture_usage",
        )
    else:
